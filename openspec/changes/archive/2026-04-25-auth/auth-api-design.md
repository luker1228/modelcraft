...
| `POST /api/auth/login` | **改造** | AuthProvider OAuth code 交换 → 手机号/用户名 + 密码直接认证 |
| `GET /api/auth/login-url` | **删除** | AuthProvider OAuth 跳转 URL，不再需要 |
...
paths:
  ...
  # /api/auth/login-url 移除（AuthProvider 不再需要）
  /api/auth/logout:
...
**向后兼容**：
- [x] 移除 AuthProvider 专属端点 `/api/auth/login-url`
- [x] `LoginRequest` 从 AuthProvider OAuth 改为双标识符+密码（破坏性变更，PRD 预期）
