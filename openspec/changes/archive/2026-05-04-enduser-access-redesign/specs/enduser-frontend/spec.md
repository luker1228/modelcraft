# Spec：EndUser 前端页面重设计

**能力领域**：`enduser-frontend`  
...
### Requirement: Org 侧用户详情页的「授权」功能

**Reason**：授权（分配 Project + Role）统一收归 Project 层管理，Org 侧只展示只读视图。Grant 弹窗和相关 API 调用废弃。

**Migration**：
- 删除「授权」按钮及 Grant 弹窗组件
- 删除对 `GRANT_PROJECT_END_USER_ACCESS`、`UPDATE_PROJECT_END_USER_ACCESS`、`REVOKE_PROJECT_END_USER_ACCESS`、`LIST_PROJECT_END_USER_ACCESS` GraphQL 文档的引用
- 「项目访问」区域改为只读，数据源切换为 `endUserRoleAssignments(endUserId)` 并按 `projectSlug` 分组展示

### Requirement: Project 访问控制页基于 ProjectAccess 的授权流程

**Reason**：原页面使用 `grantEndUserProjectAccess` 新增用户并绑定 Bundle。废弃后，页面改为通过 `assignEndUserRole` 添加用户。

**Migration**：
- 「添加用户」弹窗从调用 `grantEndUserProjectAccess` 改为调用 `assignEndUserRole`
- 列表数据源从 `listProjectEndUserAccess` 改为 `listProjectEndUserRoleUsers`
- 删除 `src/api-client/rbac/graphql-docs.ts` 中的 4 个旧 GraphQL 文档常量
