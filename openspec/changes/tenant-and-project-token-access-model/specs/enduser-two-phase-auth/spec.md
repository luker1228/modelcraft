...
在本变更中，该令牌的语义 SHALL 收敛为 `project access token`，用于访问统一 workspace 下的 project 级能力。

#### Scenario: 登录成功即返回项目访问级令牌
- **WHEN** EndUser 提交正确用户名与密码，且存在至少一个可访问项目
- **THEN** 系统 MUST 在登录响应中返回 `project access token`

### Requirement: 系统必须返回 EndUser 有权限访问的全部项目
系统 SHALL 在登录成功响应中返回该 EndUser 在当前组织下有权限访问的完整项目列表。

该响应 SHOULD 同时包含进入统一 workspace 所需的最小上下文信息，使前端可直接进入 `/org/{orgName}/workspace` 并展示 project 列表。
...
在本变更中，前端 SHALL 基于统一 workspace 容器完成 project 选择，而不是跳转到独立 end-user 页面树。
