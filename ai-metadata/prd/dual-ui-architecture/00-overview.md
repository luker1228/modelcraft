...
| 角色 | 描述 | 登录方式 |
|------|------|---------|
| Org Owner | 创建 Org，管理成员，拥有全部权限 | `/tenant/login` → 用户名/密码登录 |
| Org Admin | 管理成员、EndUser 账号、数据库集群 | `/tenant/login` → 用户名/密码登录 |
| Developer | 设计模型、配置字段、管理枚举、设置 RBAC | `/tenant/login` → 用户名/密码登录 |
...
| `aud` 值 | 颁发场景 | 可访问路由 |
|----------|---------|-----------|
| `tenant` | 租户端用户名/密码登录成功后颁发 | `/org/[orgName]/...`、`/tenant/login` |
...
- **登录入口**：`/tenant/login` → 用户名/密码表单
- **登录后**：`/org-selector` → 选择 Org → 进入 `/org/[orgName]/projects`

### 路由结构

```
/tenant/login                                   # 用户名/密码登录入口（租户专属）
...
| [05-backend-api-contract.md](./05-backend-api-contract.md) | 后端 API 契约：tenant auth 路径迁移 + end-user-auth.yaml 变更 + Org GraphQL 新增查询 |
