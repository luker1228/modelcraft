...
归属：`api/graph/org/schema/end_user.graphql`（追加到现有文件）
...
# 由 EndUser 身份调用（aud=end-user Token），不需要租户端权限。
...
**Endpoint：** `POST /graphql/org/{orgName}/`（Org GraphQL）

**Token 要求：** `aud=end-user`（中间件校验，拒绝 `aud=tenant` Token）

---

### 3. GraphQL — Runtime 数据操作（已有，需确认）
...
**GraphQL**
- [x] 新增 `listEndUserAccessibleProjects`（Workspace 动态刷新用）
