...
### 3. 认证委托给 Casdoor，ModelCraft 不自建用户体系

**决定**：用户身份由 Casdoor 管理，ModelCraft 只持有 `ExternalID`（来自 JWT.sub），不存储密码。

**理由**：
- 认证是通用基础设施，不是 ModelCraft 的核心竞争力
- Casdoor 提供完整的 SSO、OAuth2、OIDC 支持
- ModelCraft 只关心"这个用户在哪个 Org 里有什么权限"

**影响**：
- `User` 实体通过 `ExternalID` 与 Casdoor 用户关联
- JWT token 由 Casdoor 签发，ModelCraft 只做验证
