...
### 3. ModelCraft 自建用户体系，使用用户名/密码认证

**决定**：用户身份由 ModelCraft 自身管理，使用用户名 + 密码登录，JWT 由 ModelCraft Gateway 自行签发（ES256）。

**理由**：
- 当前阶段不依赖外部 AuthProvider，降低部署复杂度
- 认证逻辑完全自控，便于快速迭代

**影响**：
- `User` 实体由 ModelCraft 直接持有并管理
- JWT token 由 Gateway 签发，issuer 为 `mc-platform`
- Developer（租户端）和 EndUser（用户端）均使用用户名/密码登录，无 OAuth 回调流程
