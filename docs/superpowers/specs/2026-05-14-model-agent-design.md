...
Next.js BFF: /api/copilotkit/route.ts   ← 新建
  - 代理请求到 Python Agent 服务
  - 从 session 读取 JWT，注入 Authorization header
...
├── client/
│   └── graphql_client.py       # Backend GraphQL HTTP client（httpx + JWT 透传）
├── config.py                   # 环境变量配置
├── requirements.txt
└── Dockerfile
```
...
async def query_model(
    ...
    """查询指定模型的数据，返回记录列表。"""
    # 调用 Backend GraphQL Runtime API
    # 透传 JWT Token（从 CopilotKit context 获取）
```
...
使用 `@copilotkit/backend` SDK 提供的 handler，代理到 Python 服务并注入 JWT。
