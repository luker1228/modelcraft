...
export default function RuntimeRecordWorkspace({
  ...
  const accessAdapter = useMemo<RecordAccessAdapter | null>(() => {
    ...
    return {
      ...
      createRuntimeClient: (_databaseName: string, _modelName: string) => {
        if (!token) {
          throw new Error('RuntimeRecordWorkspace: end-user token is not available')
        }
        return createEndUserScopedClient(orgName, projectSlug, token)
      },
    }
  ...
  // runtime client 始终使用 end-user scoped endpoint
