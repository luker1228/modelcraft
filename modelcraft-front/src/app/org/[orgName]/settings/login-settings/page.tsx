...
export default function OrgLoginSettingsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-foreground">登录配置</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          配置 Org 级终端用户登录能力与会话策略
        </p>
      </div>
      ...
      <div className="rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-700">
        <div className="flex items-start gap-2">
          ...
          <p>
            当前页面已接入路由与导航，保存动作暂为前端占位。后续可直接对接 Org 登录配置 API。
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between rounded-md border border-border p-4">
          <div>
            <Label className="text-base">启用终端用户登录</Label>
            <p className="mt-1 text-sm text-muted-foreground">
      ...
      <div className="rounded-md border border-border bg-muted/30 p-4 text-sm text-muted-foreground">
        End-User 登录地址：
        <span className="ml-1 font-mono text-foreground">{endUserLoginPath}</span>
      </div>
    </div>
  )
}
