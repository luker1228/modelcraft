...
| 8 | 无法删除非本人的记录（EndUser 尝试删除） | USING filter 过滤，pre-SELECT 0 行，返回 REPO_NOT_FOUND | `SELECT * FROM orders WHERE ((id=?) AND ((user_id=?))) args=[..., rls-test-user-001]` → 0 行；见下方错误详情 ↓ | ✅ |
...
| 21 | admin 角色可更新 products 记录（非存在记录则无副作用） | UPDATE WHERE `(id=?) AND (1=1)` 执行，post-SELECT 0 行，返回 SYSTEM_ERROR | `UPDATE products SET name=? WHERE ((id=?) AND ((1=1)))` → `SELECT * FROM products WHERE ((id=?) AND ((1=1)))` → 0 行；返回 `[SYSTEM_ERROR] System error: {0}` | ✅ |
...
| 28 | assignee 尝试更新 reporter 创建的 task | USING filter 仅匹配 assignee_id，UPDATE 0 行，post-SELECT 0 行，返回 SYSTEM_ERROR | `UPDATE tasks SET title=? WHERE ((id=?) AND ((reporter_id=?) OR (assignee_id=?))) args=[..., rls-assignee-002, rls-assignee-002]` → `SELECT * FROM tasks WHERE ...` → 0 行；返回 `[SYSTEM_ERROR] System error: {0}` | ✅ |
| 29 | reporter 创建 rls-delete-target 记录 | INSERT 成功 | `INSERT INTO tasks (id, project_id, reporter_id, title) VALUES (...)` | ✅ |
| 30 | assignee 尝试删除 reporter 创建的 task | pre-SELECT 按 reporter_id=assignee 查，0 行，返回 REPO_NOT_FOUND | `SELECT * FROM tasks WHERE ((id=?) AND ((reporter_id=?))) args=[..., rls-assignee-002]` → 0 行；返回 `[REPO_NOT_FOUND] sql: no rows in result set` | ✅ |
...
| 38 | 更新 non-existent-id 评论（USING 过滤，0 行受影响） | UPDATE 执行，post-SELECT 0 行，返回 SYSTEM_ERROR | `UPDATE task_comments SET content=? WHERE ((id=?) AND ((author_id=?)))` → `SELECT * FROM task_comments WHERE ((id=?) AND ((author_id=?)))` → 0 行；返回 `[SYSTEM_ERROR] System error: {0}` | ✅ |
| 39 | 删除 non-existent-id 评论（USING 过滤，0 行受影响） | pre-SELECT 按 author_id 查，0 行，返回 REPO_NOT_FOUND | `SELECT * FROM task_comments WHERE ((id=?) AND ((author_id=?))) args=[non-existent-id, rls-test-user-001]` → 0 行；返回 `[REPO_NOT_FOUND] sql: no rows in result set` | ✅ |
...
**GraphQL 响应**：
```json
{"data":{"delete":null},"errors":[{"message":"[REPO_NOT_FOUND] sql: no rows in result set","locations":[{"line":1,"column":12}],"path":["delete"]}]}
```

**审计结论**：✅ USING filter 正确过滤——pre-SELECT 未找到行，DELETE 不执行。错误码 `REPO_NOT_FOUND` 对调用方表现为"记录不存在"，不泄露「被 RLS 过滤」的信息，符合安全设计。
...
**GraphQL 响应**：
```json
{"data":{"update":null},"errors":[{"message":"[SYSTEM_ERROR] System error: {0}","locations":[{"line":1,"column":12}],"path":["update"]}]}
```

**审计结论**：⚠️ `[SYSTEM_ERROR] System error: {0}` 是 UPDATE + post-SELECT 后找不到行时后端抛出的内部错误，错误消息模板中 `{0}` 占位符未被替换，属于格式化 Bug。RLS 行为本身正确（admin `1=1` policy 通过，SQL 执行，但记录不存在）。该错误消息不够友好，但不影响功能正确性。
...
**审计结论**：⚠️ 同 #21，UPDATE 执行后 post-SELECT 返回 0 行触发 SYSTEM_ERROR。RLS 行为正确（USING filter `reporter_id OR assignee_id` 中 assignee 不匹配该 task 的 reporter，0 行受影响）。`{0}` 格式化 Bug 同 #21。
...
**审计结论**：✅ 同 #8，pre-SELECT 用 reporter_id=assignee 查不到行，DELETE 不执行，返回 REPO_NOT_FOUND。对调用方表现为"记录不存在"，不泄露 RLS 过滤信息。
...
**审计结论**：⚠️ 同 #21/#28，UPDATE 后 post-SELECT 返回 0 行触发 SYSTEM_ERROR。RLS 行为正确（USING filter `author_id=current-user` 过滤，non-existent-id 无论如何都不存在）。
...
**审计结论**：✅ 同 #8/#30，pre-SELECT 用 author_id=current-user 查不到行，DELETE 不执行，返回 REPO_NOT_FOUND。
...
| USING filter pre-SELECT 0 行（delete） | `[REPO_NOT_FOUND] sql: no rows in result set` | 先 SELECT 后 DELETE，SELECT 返回 0 行则不执行 DELETE |
| USING filter 使 UPDATE 后 post-SELECT 0 行 | `[SYSTEM_ERROR] System error: {0}` | UPDATE 执行完 post-SELECT 0 行触发；`{0}` 是格式化 Bug，不影响 RLS 正确性 |
