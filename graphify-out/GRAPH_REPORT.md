...
- `writeError()` --calls--> `writeJSON()`  [EXTRACTED]
  modelcraft-gateway/internal/proxy/handler.go → modelcraft-backend/internal/interfaces/http/handlers/auth/handler.go
- `proxyBackendError()` --calls--> `writeError()`  [EXTRACTED]
  modelcraft-gateway/internal/auth/handler.go → modelcraft-gateway/internal/proxy/handler.go
...
- **Thin community `Community 712`** (1 nodes): `public.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 713`** (1 nodes): `graphql-docs.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
