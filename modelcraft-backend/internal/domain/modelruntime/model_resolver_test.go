...
// TestResolveEnumLabelSourceFieldName tests the resolveEnumLabelSourceFieldName function
// for various edge cases around enumRelationId and metadata.
func TestResolveEnumLabelSourceFieldName(t *testing.T) {
	enumRelationID := "rel-123"
	...
	tests := []struct {
		...
		{
			name: "valid metadata with enumRelationSourceField key",
			field: &RuntimeField{
				Name:           "status_label",
				EnumRelationID: &enumRelationID,
				Metadata: map[string]any{
					"enumRelationSourceField": "status",
				},
			},
			wantSource: "status",
			wantErr:    false,
		...
		{
			name: "valid metadata with sourceFieldName key (fallback)",
			field: &RuntimeField{
				Name:           "priority_label",
				EnumRelationID: &enumRelationID,
				Metadata: map[string]any{
		...
		{
			name: "valid metadata with sourceField key (fallback)",
			field: &RuntimeField{
				Name:           "type_label",
				EnumRelationID: &enumRelationID,
				Metadata: map[string]any{
		...
		{
			name: "enumRelationId present but metadata is nil",
			field: &RuntimeField{
				Name:           "missing_meta_label",
				EnumRelationID: &enumRelationID,
				Metadata:       nil,
			},
			wantErr:     true,
		...
		{
			name: "enumRelationId present but metadata is empty map",
			field: &RuntimeField{
				Name:           "empty_meta_label",
				EnumRelationID: &enumRelationID,
				Metadata:       map[string]any{},
			},
			wantErr:     true,
			errContains: "cannot resolve source field",
		},
		{
			name: "metadata has key but value is nil",
			field: &RuntimeField{
				Name:           "nil_value_label",
				EnumRelationID: &enumRelationID,
				Metadata: map[string]any{
					"enumRelationSourceField": nil,
				},
			},
			wantErr:     true,
			errContains: "cannot resolve source field",
		...
		{
			name: "metadata has key but value is empty string",
			field: &RuntimeField{
				Name:           "empty_value_label",
				EnumRelationID: &enumRelationID,
				Metadata: map[string]any{
					"enumRelationSourceField": "",
				},
			},
			wantErr:     true,
			errContains: "cannot resolve source field",
		...
		{
			name: "metadata has key but value is not a string (integer)",
			field: &RuntimeField{
				Name:           "int_value_label",
				EnumRelationID: &enumRelationID,
				Metadata: map[string]any{
					"enumRelationSourceField": 123,
				},
			},
			// cast.ToStringE can convert int to string "123", so this will actually pass
			wantSource: "123",
			wantErr:    false,
		},
		{
			name: "enumRelationId is nil",
			field: &RuntimeField{
				Name:           "no_relation_label",
				EnumRelationID: nil,
				Metadata: map[string]any{
					"enumRelationSourceField": "status",
				},
			...
			wantErr:     true,
			errContains: "enumRelationId is required",
		},
		{
			name: "enumRelationId is empty string",
			field: func() *RuntimeField {
				empty := ""
				return &RuntimeField{
					Name:           "empty_relation_label",
					EnumRelationID: &empty,
					Metadata: map[string]any{
						"enumRelationSourceField": "status",
					},
				}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			source, err := resolveEnumLabelSourceFieldName(tt.field)
