...
func TestJWTSigner_IssueAccessToken(t *testing.T) {
	...
	t.Run("valid tenant token", func(t *testing.T) {
		tokenStr, err := signer.IssueAccessToken("user-1", "acme", jwt.ClaimStrings{AudienceTenant}, nil)
		if err != nil {
	...
	t.Run("empty orgName returns error", func(t *testing.T) {
		_, err := signer.IssueAccessToken("user-1", "", jwt.ClaimStrings{AudienceTenant}, nil)
		if err == nil {
	...
	t.Run("end_user audience token", func(t *testing.T) {
		tokenStr, err := signer.IssueAccessToken("user-2", "acme", jwt.ClaimStrings{AudienceEndUser}, nil)
		if err != nil {
