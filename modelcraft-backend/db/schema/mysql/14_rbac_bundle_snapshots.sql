...
-- 权限包版本快照（item-centric 模型）
-- 依赖: 13_rbac_permissions.sql（end_user_permission_bundles）
-- =============================================================

-- -------------------------------------------------------------
-- 1. end_user_permission_bundle_snapshots — 权限包历史快照
--    每次 data permission item 列表变更时自动保存只读快照，最多保留最近 5 个版本
--    快照记录每个 item 的 modelId/grantType/preset/customPermissionId/sortOrder
-- -------------------------------------------------------------
CREATE TABLE `end_user_permission_bundle_snapshots` (
  ...
  `items`           JSON         NOT NULL                    COMMENT '快照时刻的 data permission item 列表，格式：[{"modelId":"uuid","grantType":"PRESET|CUSTOM","preset":"READ_ALL|...","customPermissionId":"uuid|null","sortOrder":0}]',
  ...
  COMMENT='权限包历史快照：记录每次 data permission item 列表变更，支持回滚';
