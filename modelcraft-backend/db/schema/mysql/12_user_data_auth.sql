...
CREATE TABLE IF NOT EXISTS `project_roles` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_project_roles_name`   (`org_name`, `project_slug`, `name`, `delete_token`) COMMENT 'Project 内角色名唯一',
  UNIQUE KEY `uk_project_roles_org_id` (`org_name`, `id`, `delete_token`) COMMENT '软删除唯一键保护',
...
CREATE TABLE IF NOT EXISTS `project_role_users` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_project_role_users` (`org_name`, `role_id`, `user_id`) COMMENT '用户在同一角色中不重复分配',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Project 级角色-用户关联表（纯关联，不可修改，删除重建）';
