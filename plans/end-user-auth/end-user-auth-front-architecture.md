...
| `end-user-cookie-utils.ts` | `set/get/clearEndUserRefreshTokenCookie()`，key = `end_user_refresh_token`，path 绑定到 `/org/{orgName}/project/{projectSlug}`。 |
...
> **与 `cookie-utils.ts` 的关键差异**：Cookie path 从 `/`（全局）改为 `/org/{orgName}/project/{projectSlug}`（Project 级），实现不同 Project 的终端用户 session 硬隔离。
...
| 使用正则匹配（而非 `startsWith`） | 精确匹配 project 路径段，防止误匹配 `/org/x/project/` 等中间路径 |
...
**路由**：`/org/[orgName]/project/[projectSlug]/user/login`
...
export function useEndUserLoginForm(orgName: string, projectSlug: string) {
  ...
  const onSubmit = async (values: EndUserLoginFormValues) => {
    ...
    try {
      ...
      const target = redirect ?? `/org/${orgName}/project/${projectSlug}/data`
  ...
  useEffect(() => {
    async function restoreSession() {
      ...
      if (!newToken) {
        ...
        const loginUrl = `/org/${params.orgName}/project/${params.projectSlug}/user/login`
...
export function useEndUser(): { user: EndUserInfo | null; logout: () => Promise<void> } {
  ...
  const logout = async () => {
    ...
    removeEndUserSession()
    router.replace(`/org/${params.orgName}/project/${params.projectSlug}/user/login`)
  }
...
**路由**：`/org/[orgName]/project/[projectSlug]/end-users`
